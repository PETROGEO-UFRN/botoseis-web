...
class Visualization(BaseVisualization):
    ...
    def __init__(
        ...
        for index in [1, 2]:
            ...
            self.sources[cdp_key] = ColumnDataSource(data={"image": [gained_data]})
            self.plots[cdp_key] = plotFactory(
                xAxisLabel=f"{OFFSET_BASE_TITLE}{current_gather_index + 1}",
                yAxisLabel="Time (s)" if index == 1 else "",
            )
            self.renderers[cdp_key] = offsetsImageRendererFactory(
                plot=self.plots[cdp_key],
                source=self.sources[cdp_key],
                offsets=self.gather_offsets,
                first_time_sample=FIRST_TIME_SAMPLE,
                widthTimeSamples=self.plot_options_state.width_time_samples,
            )
            NMOCurveRendererFactory(
                plot=self.plots[cdp_key],
            ...
            self.plots[semblance_key] = plotFactory(
                xAxisLabel=f"{VELOCITY_BASE_TITLE}{current_gather_index + 1}",
            )
        ...
        # Attach MouseMove for NMO hyperbola on hover
        for i in (1, 2):
            def make_hover_cb(idx):
                def cb(event):
                    v_min = self.plot_options_state.first_velocity_value
                    v_max = self.plot_options_state.last_velocity_value
                    t_min = FIRST_TIME_SAMPLE
                    t_max = FIRST_TIME_SAMPLE + self.plot_options_state.width_time_samples
                    if (v_min <= event.x <= v_max and t_min <= event.y <= t_max
                            and self.plot_options_state.is_nmo_hyperbole_on):
                        self.update_nmo_curve_source(idx, event.y, event.x)
                    else:
                        self.update_nmo_curve_source(idx, None, None)
                return cb
            self.plots[f"semblance_{i}"].on_event(MouseMove, make_hover_cb(i))
    ...
    def updateGain(self, gain: dict) -> None:
        self.gain = {**self.gain, **gain}
        self.handle_state_change()

    def updateGatherIndex(self, gatherIndex: int) -> None:
        self.plot_options_state.updatePlotOptionsState(
            gather_index_start=gatherIndex - 1,
        )
        self.handle_state_change()

    def update_nmo_hyperbole_state(self, on: bool) -> None:
        self.plot_options_state.is_nmo_hyperbole_on = on
    ...
    def update_nmo_curve_source(
        ...
        if selected_time is None or selected_velocity is None:
    ...
    def save_picks_in_memory(self):
        ...
        for index in [1, 2]:
            ...
            self.picking_data[str(current_gather_index)] = dict(
    ...
    def remove_nmo(self):
        for index in [1, 2]:
            ...
            self.sources[cdp_key].data = {"image": [gained_data]}
