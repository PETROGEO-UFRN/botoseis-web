...
import React__default, { isValidElement, cloneElement, Children, useCallback, useRef, useState, useEffect } from "react";
import { create } from "zustand";
import { a as clsx, g as generateUtilityClass, d as generateUtilityClasses, b as composeClasses, s as styled$1, m as memoTheme, e as createSimplePaletteValueFilter, C as ClassNameGenerator, r as rootShouldForwardProp } from "./createSimplePaletteValueFilter-DjRAvOB9.js";
import { s as styled, k as styleFunctionSx, u as useTheme$1, e as extendSxProp, l as useEnhancedEffect, n as defaultTheme$1, T as THEME_ID, f as useDefaultProps, j as capitalize, o as alpha, p as getOverlayAlpha, q as useId, t as darken, v as lighten, w as createTheme, r as resolveProps, R as Route } from "./router-D2L1tGvJ.js";
...
const BOKEH_BRIDGE_MODEL_NAME = "BRIDGE_ACTIONS_TRIGGER_MODEL";
const BOKEH_BRIDGE_FEEDBACK_MODEL_NAME = "BRIDGE_FEEDBACK_MODEL";
const plotAPI = {
  baseURL: "http://localhost:5006",
  path: "/api/bokeh-script"
};
async function getPlotScript({
  plotType,
  workflowId,
  setupProps
}) {
...
const useBokehDocumentStore = create((set) => ({
  bokehDocument: null,
  setBokehDocument: (document2) => set({ bokehDocument: document2 })
}));
const RETRY_MS = 500;
const MAX_RESOLVE_ATTEMPTS = 20;
function useBokehLoader(containerRef) {
  ...
  const resolveBokehDocument = useCallback(async () => {
    ...
    return await new Promise((resolve) => {
      const tryResolveDocument = () => {
        ...
        const document2 = window.Bokeh.documents.findLast(
          (document3) => document3.get_model_by_name(BOKEH_BRIDGE_MODEL_NAME)
        );
        if (document2) {
          setBokehDocument(document2);
          return resolve(document2);
        }
        return setTimeout(tryResolveDocument, RETRY_MS);
      };
      tryResolveDocument();
  ...
  const loadBokehScripts = useCallback(
    async ({
      ...
      try {
        ...
        const document2 = await resolveBokehDocument();
        if (!document2) return { ok: false };
        return { ok: true };
      } catch (error) {
...
const CONNECTION_RETRY_TIMEOUT_MS = 5e3;
function BokehPlot({
  plotType,
  workflowId,
  setupProps
  ...
  useEffect(() => {
    ...
    loadBokehScripts({ plotType, workflowId, setupProps }).then((result) => {
      ...
      console.warn(
        `Connection failed, retrying in ${CONNECTION_RETRY_TIMEOUT_MS / 1e3}s...`
      );
  ...
  return /* @__PURE__ */ jsxs(Container, { children: [
    loading && !requestLog && /* @__PURE__ */ jsx(CircularProgress, { size: 24 }),
    requestLog && /* @__PURE__ */ jsx(Alert, { severity: "info", sx: { mb: 2 }, children: requestLog }),
    /* @__PURE__ */ jsx(PlotBox, { ref: containerRef, id: "bokeh-plot-container" })
  ] });
}
var PLOT_TYPES_ENUM = /* @__PURE__ */ ((PLOT_TYPES_ENUM2) => {
  PLOT_TYPES_ENUM2["SAMPLE_PLOT"] = "sample-plot";
  return PLOT_TYPES_ENUM2;
})(PLOT_TYPES_ENUM || {});
const DEBOUNCE_TIME_MS = 500;
const THROTTLE_TIME_IN_MS = 41;
function useBokeh() {
  const bokehDocument = useBokehDocumentStore((state) => state.bokehDocument);
  const pendingPromises = useRef(/* @__PURE__ */ new Map());
  const newValuesBufferRef = useRef({});
  ...
  const emitDebouncedTrigger = (newValues) => {
  ...
  const emitTrigger = (newValues) => {
    if (!bokehDocument)
      return console.warn("[useBokeh]: Document not ready yet.");
    if (!newValues) return;
    const trigger = bokehDocument.get_model_by_name(
  ...
  useEffect(() => {
    if (!bokehDocument)
      return console.warn("[useBokeh]: Document not ready yet.");
    const feedbackSource = bokehDocument.get_model_by_name(
...
function SamplePage() {
  ...
  return /* @__PURE__ */ jsxs(Box, { sx: {
    ...
    /* @__PURE__ */ jsx(Button, { variant: "contained", onClick: handlePing, disabled: pinging, children: pinging ? /* @__PURE__ */ jsx(CircularProgress, { size: 18 }) : "Ping" }),
    /* @__PURE__ */ jsx(BokehPlot, { plotType: PLOT_TYPES_ENUM.SAMPLE_PLOT, workflowId })
  ] });
}
export {
  SamplePage as component
};
